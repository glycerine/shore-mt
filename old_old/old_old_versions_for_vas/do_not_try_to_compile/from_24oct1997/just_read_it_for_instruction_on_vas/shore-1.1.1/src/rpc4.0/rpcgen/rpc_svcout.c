	...
	char *storage;
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		for (proc = vp->procs; proc != NULL; proc = proc->next) {
			...
			if(ansi) {
				f_print(fout, "\t\t%s = (char *(*)(void *, void *)) ", ROUTINE);
			} else {
...
#ifndef notdef
		/* FREE RESULT AFTER IT'S SENT */
		/* Assumes that it's not stolen for retransmission
		 * by the lower layer
		 */
 		f_print(fout, "\tif(result){\n");
		printif("freeargs", TRANSP, "", RESULT);
 		f_print(fout, "\t\t(void)fprintf(stderr, \"unable to free result\\n\");\n");
		f_print(fout, "\t\texit(1);\n");
		f_print(fout, "\t}\n\t}\n");
#endif
