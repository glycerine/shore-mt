...
#ifndef SDL_SET_H
#define SDL_SET_H 1
// base class for set representation.  Preliminary take: sets are
// a variable-size array of ocrefs, with the space for the array
// allocated in the heap.  Type binding will be implemented via
// templates or a wrapper class.
extern void 
sdl_heap_op(HeapOps op, void **spt, size_t len, int free_it);
#if (__GNUC_MINOR__ < 7)
#   include <memory.h>
#else
...
class sdl_set  : public sdl_heap_base {
private:
public:
	unsigned int  num_elements; // current number of elements
	void *get_space() { return space; };
	void __apply(HeapOps op);
	void Apply(class app_class *) const;
#ifdef oldcode
	OCRef * space_pt() const { return  (OCRef *)space; };
	void add_elt(const OCRef &);
	void add_unique_elt(const OCRef &);
	void delete_elt(const OCRef &);
	OCRef delete_one() 
	{ if (num_elements>0) return ((OCRef *)space)[--num_elements];
		else return OCRef(0);
	}
	bool member(const OCRef &) const;
	// also need some form of iteration???
	OCRef get_elt(int i) const { 
		if (i<num_elements && i>=0) 
			return space_pt()[i] ;
		else 
			return OCRef(0);
	}
	size_t max_elements() const { return cur_size / sizeof(OCRef); }
	size_t get_size() const { return num_elements; };
	void resize(int i = 4);
#endif
};
...
{	public:	
	...
	void del(const Ref<T> &arg);
	Ref<T> delete_one();
	void del_from_inverse(const Ref<T> & arg) { Bag< Ref<T> >::del(arg); };
	void add_from_inverse(const Ref<T> & arg) { Bag< Ref<T> >::add(arg); };
...
void SET_INV(T,TCONT,TC_OFFSET)::add(const REF(T) & arg) {			\
	if (arg != NULL) {						\
		Set<Ref<T> >::add(arg);					\
...
void SET_INV(T,TCONT,TC_OFFSET)::del(const REF(T) & arg)	{			\
	if (arg!= NULL)							\
	{								\
		Set<Ref<T> >::del(arg);					\
...
void BAG_INV(T,TCONT,TC_OFFSET)::add(const REF(T) & arg) {			\
	if (arg != NULL) {						\
		Bag<Ref<T> >::add(arg);					\
		arg.update()->TINV.add_from_inverse(REF(TCONT)(get_contpt()));\
	}								\
...
/* del perhaps needs some error checking */				\
void BAG_INV(T,TCONT,TC_OFFSET)::del(const REF(T) & arg)	{			\
	if (arg!= NULL)							\
	{								\
		Bag<Ref<T> >::del(arg);					\
		arg.update()->TINV.del_from_inverse(REF(TCONT)(get_contpt()));\
...
template <class t>
class Apply
// a little thing to pass around & do the heap management functions.
// By default, do nothing,as in the Apply template below.  But,
// if some sdl-defined type x has an __apply fct, we will override
// the do-nothing apply with an explicit definition of the class
// as class Apply<tc> : public DoApply<tc>{}; a b
// note: if we do byte swapping through this mechanism, we may
// want to provide explicit Apply<t>'s for all primitive types??
{
	public:
		static void __apply(t * tpt,HeapOps o) { tpt->__apply(o); }
};
#define NO_APPLY(tn) struct Apply<tn> { static void __apply(tn *,HeapOps) {} };
NO_APPLY(char)
NO_APPLY(short)
NO_APPLY(long)
NO_APPLY(float)
NO_APPLY(double)
NO_APPLY(unsigned char)
NO_APPLY(unsigned short)
NO_APPLY(unsigned long)
NO_APPLY(bool)


#ifdef old_code
// null apply template, used by default.
template <class t>
class Apply
{
public:
	static void __apply(t *tpt,HeapOps o) {};
};

#endif
	
template <class t>
class Sequence : protected sdl_heap_base {
protected:
	// sloppily, we steal a bit for an allocation flag.
	// unsigned int free_it:1;
	// unsigned int max_elements:30; // size of the OCRef array
	// t * space;
	t * space_pt() const { return (t *)space; };
	size_t num_elements; // current number of elements
	size_t max_elements() const { return cur_size / sizeof(t); }
public:
	// constructors
	Sequence() 
	{	if (cur_size==0) num_elements = 0; } 
	// note: if cur_size != 0 implies that the sdl_heap_base ctor leaf
	//	cur_size != 0, which in turn implies that the constructor
	//	was called within the sdl runtime, and didn't do anything;'
	//	nothing should be done here either.
	// note: sdl_heap_base ctor was called first.
	Sequence(const Sequence<t> & cseq) {
		sdl_heap_base::init(); // clear the base.
		num_elements = 0;
		set_size(cseq.get_size());
		for (size_t j=0; j<num_elements; j++) space_pt()[j]=cseq.space_pt()[j];
	}
	void _destroy_it() 
	{ 
		if (free_it) // call del here so it is type-specific
		{
			delete  [] space_pt();
		}
		__reset_space(0,0); // don't want ~sdl_heap_base to free it.
	}
	~Sequence() { _destroy_it(); }
	Sequence<t> & 
	// try destruction, then construction.
	operator=(const Sequence<t> & rhs)
	{
		// this->~Sequence();
		_destroy_it();
		new((void *)this) Sequence<t>(rhs);
		return *this;
	}
			
		
	int append_elt(const t & new_elt) { // initialized element
	 	if (num_elements>=max_elements()) resize();
		space_pt()[num_elements] = new_elt;
		++num_elements;
		return (int)(num_elements -1);
	}
	int append_elt() { // unitilialized element.
	 	if (num_elements>=max_elements()) resize();
		++num_elements;
		return (int) (num_elements -1);
	}
	void delete_elt(unsigned int i) {
		for (int j=i; j<int(num_elements)-1; j++) 
			space_pt()[j]=space_pt()[j+1];
		if (i<num_elements) --num_elements;
	}
	// also need some form of iteration???
	const t & get_elt(unsigned int i) const { 
		if (i<num_elements) 
			return space_pt()[i] ;
		// refs used to return null here.
		// this function really has to go, need a return value
		// status somewhere.
		// for refs at least, constructor will return
		// a null.  but, will also warn. yuckuck
		static t tmp; 
		return tmp;
	}
	const t & operator[] (unsigned int i) const { return get_elt(i);}
	// also provide a r/w version of get_elt to allow updates.

	//t & write_elt(unsigned int i) { return (t &)(this->get_elt(i));}
	// cast out constness from get_elt.
	// for some odd reason get_elt call needs this, else a compiler
	// error occurs.
	// e.g. { return (t &)get_elt(i);} doesn't work sometime.
	// yuck, the previous fix didn't really work so we needlessly
	// duplicate get_elt code for write_elt.
	// also need some form of iteration???

	t & write_elt(unsigned int i) const { 
		if (i<num_elements) 
			return space_pt()[i] ;
		static t tmp; 
		return tmp;
...
protected:
	void resize(int i = 4) {
		int new_maxelts =  max_elements() + i;
		t * new_space  = new t[new_maxelts + i];
		if (!new_space ) abort();
		for (size_t j=0; j<num_elements; j++)
			new_space[j] = space_pt()[j];
		if (free_it && space_pt())
			delete [] space_pt();
		__reset_space(new_maxelts * sizeof(t),new_space);
		free_it = 1;
	}
public:
	void set_size(int i) // set size absolutely.
	{
		if (max_elements() < (size_t)i) // need to resize
			resize(i- max_elements());
		num_elements = (size_t) i;
	}
	void __apply(HeapOps op) { 
		if (op==PrepareForDisk)
			__trim_space(max_elements()*sizeof(t));
		sdl_heap_base::__apply(op);
		for (size_t i=0; i<num_elements; i++)
			::Apply<t>::__apply(&space_pt()[i],op);
	}; // if elt has no heap_apply
};

template <class t> 
class Set : public Sequence<t>
// rewrap the interface to make Sequences look like sets.
{
public:
	Set() {} // just do Sequence ctor.
	Set(const Set<t> &cset) :Sequence<t>(cset) {} // ditto.
	// ~Set(); // {} // again, just do Seqence dtor.
	Set<t> &
	operator=(const Set<t> &cet) // again, as a sequence.
	{
		this->Sequence<t>::operator=(cet);
		return *this;
	}
	// index used to be in Sequence<T>, but that requires T
	// to have operator==...
	int index(const t & elt) const // look for a particular element.
	{
		for (size_t j=0; j<num_elements; j++)
			if (space_pt()[j]==elt)
				return (int)j;
	...
	void add(const t & new_elt) // add after checking for duplicate
	{
		if (index(new_elt) <0)
			append_elt(new_elt);
	}
	void del(const t & arg) {
		int di = index(arg);
		if (di>=0)	delete_elt(di);
	}
	t get_elt(int i) const { return Sequence<t>::get_elt(i);}
	bool member(const t &arg) const { return (index(arg) >= 0);}
	size_t get_size() const { return Sequence<t>::get_size();}
	void set_size(int i) { Sequence<t>::set_size(i);}
	t delete_one() {
		t rval;
		if (get_size()>0) {
			rval = get_elt(get_size()-1);
			delete_elt(get_size()-1);
		}
		return rval;
	}
	void __apply(HeapOps op) { Sequence<t>::__apply(op);}
};

template <class t> 
class Bag : public Set<t>
// Like set, except for unique check on add member.
{
public:
	Bag() {} // just do Set ctor.
	Bag(const Bag<t> &cset) :Set<t>(cset) {} // ditto.
	~Bag() {} // again, just do Set dtor.
	Bag<t> &
	operator=(const Bag<t> &cet) // again, as a sequence.
	{
		this->Set<t>::operator=(cet);
		return *this;
	}
	void add(const t & new_elt) // add withoug checking for duplicate
	{ 	
		append_elt(new_elt);
	}
};
...
#define sdl_val_set Sequence
#define sdl_val_set_apply Sequence
#endif
