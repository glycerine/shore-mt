...
static bool_t	xdrrec_getshort();
static bool_t	xdrrec_putshort();
static bool_t	xdrrec_getbyte();
static bool_t	xdrrec_putbyte();
	...
	int (*writeit)();  /* like write, but pass it a tcp_handle, not sock */
{
	...
	if (rstrm->the_buffer == NULL) {
		...
		memset(rstrm->the_buffer, '\0',
			sendsize + recvsize + BYTES_PER_XDR_UNIT);
#endif
	}
	...
	rstrm->out_finger = rstrm->out_boundry = rstrm->out_base;
		if((u_int)(rstrm->out_finger) & 0x3) { stop(); }
...
static bool_t
xdrrec_getshort(xdrs, lp)
	XDR *xdrs;
	short *lp;
{
	long  	l;
	bool_t res;
	res = xdrrec_getlong(xdrs, &l);
	*lp = (short) l;
	return res;
}
static bool_t
xdrrec_putshort(xdrs, lp)
	XDR *xdrs;
	short *lp;
{
	long l = *lp;
	return xdrrec_putlong(xdrs, &l);
}
static bool_t
xdrrec_getbyte(xdrs, lp)
	XDR *xdrs;
	char *lp;
{
	long  	l;
	...
	register u_int len;
{
	...
	while (len > 0) {
		...
		if((u_int)(rstrm->out_finger + current) & 0x3) { stop(); }
