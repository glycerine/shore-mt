...
struct ct_data {
	...
	bool_t          ct_nowait;       /* wait removed by clnt_control? */
...
#ifdef NOTDEF
	/* now done in the svas code */
		if (*sockp >= 0) {
			/* set socket opts */
			int	 optlen = sizeof(int);
			int  optval = 0x01010101;
			if(setsockopt(*sockp, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen)<0) {
				rpc_createerr.cf_stat = RPC_SYSTEMERROR;
				rpc_createerr.cf_error.re_errno = errno;
				(void)close(*sockp);
				goto fooy;
	...
	int len;
{
	...
	for (cnt = len; cnt > 0; cnt -= i, buf += i) {
		/*
		fprintf(stderr, "writetcp on sock %d, %d bytes\n", ct->ct_sock, cnt);
		*/
