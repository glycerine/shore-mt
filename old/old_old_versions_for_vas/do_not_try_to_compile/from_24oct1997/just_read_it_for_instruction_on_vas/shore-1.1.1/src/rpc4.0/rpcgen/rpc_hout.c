	...
	definition *def;
{
	...
	if (def->def_kind != DEF_PROGRAM && def->def_kind != DEF_CONST) {
		...
		f_print(fout, "bool_t xdr_%s", def->def_name);
		if(ansi) {
			f_print(fout, "(XDR  *, %s);\n", def->def_name);
		} else {
			f_print(fout, "();\n");
...
{
	if(ansi) {
		if(strncmp(def->def.co,"0x",2)==0) {
			f_print(fout, "const unsigned int %s = %s;\n", def->def_name, def->def.co);
		} else if(strncmp(def->def.co,"0",2)==0) {
			f_print(fout, "const unsigned int %s = %s;\n", def->def_name, def->def.co);
		} else {
			f_print(fout, "const int %s = %s;\n", def->def_name, def->def.co);
		}
	} else {
