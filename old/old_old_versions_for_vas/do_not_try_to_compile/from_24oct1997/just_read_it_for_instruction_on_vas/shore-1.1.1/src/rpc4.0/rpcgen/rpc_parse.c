...
get_definition()
{
	...
	defp->is_packed = false;
	get_token(&tok);

	if(tok.kind == TOK_PACKED) {
		dprint(stderr, "%d: packed", __LINE__);
		defp->is_packed = true;
		get_token(&tok);
	}
	switch (tok.kind) {
	...
	defkind dkind;
{
	...
	*prefixp = NULL;
	...
	case TOK_BOOL:
		*typep = tok.str;
		dprint(stderr, 
			"%d: tok.kind=%d, *typep=%s\n", __LINE__, tok.kind,*typep);
		break;
	default:
