                            ...
                            q2_input_t&  q2in )
{
    ...
    table_row_t* prpart = _ppart_man->get_tuple();
    assert(prpart);
	...
	guard<table_scan_iter_impl<part_t> > p_iter;
	{
	    ...
	    e = _ppart_man->get_iter_for_file_scan(_pssm, tmp_p_iter);

	    p_iter = tmp_p_iter;

	    if(e.is_error()){ goto done1;}
	}
	    
	bool eof;
	tpch_part_tuple apart;

	e = p_iter->next(_pssm, eof, *prpart);
	if( e.is_error() ){ goto done1;}
	...
	while(!eof){
	    ...
	    if( size == q2in.p_size &&
		strstr(apart.P_TYPE, types3) != NULL){
		//partK.insert(pair<int,bool>(apart.P_PARTKEY, true));
		vector<int>* v = new vector<int>();
		minlist.insert( pair<int, pair< decimal, vector<int>* > >(apart.P_PARTKEY,
									  pair<decimal, vector<int>* >(-1, v)));
	    }
 ...
 done1:
    _ppart_man->give_tuple(prpart);

    //Hey!
    //table scan nation: r_key = q2in.key
    map<int,bool> nationK;
    
    table_row_t* prnation = _pnation_man->get_tuple();
    assert (prnation);
    ...
    
    {
        guard< table_scan_iter_impl<nation_t> > n_iter;
        {
            ...
            e = _pnation_man->get_iter_for_file_scan(_pssm, tmp_n_iter);
            n_iter = tmp_n_iter;
            if (e.is_error()) { goto done2; }
        }
	...
	bool eof;
        e = n_iter->next(_pssm, eof, *prnation);
...
done2:
    _pnation_man->give_tuple(prnation);

    //table scan supplier
    map<int, bool> suppK;
    
    table_row_t* prsupp = _psupplier_man->get_tuple();
    assert(prsupp);
	...
	guard<table_scan_iter_impl<supplier_t> > s_iter;
	{
	    ...
	    e = _psupplier_man->get_iter_for_file_scan(_pssm, tmp_s_iter);

	    s_iter = tmp_s_iter;

	    if(e.is_error()){goto done4;}
 ...
 done4:
    _psupplier_man->give_tuple(prsupp);
    //cout<<suppK.size()<<endl;
    
    //table scan partsupp
        
    
    table_row_t* prpartsupp = _ppartsupp_man->get_tuple();
    assert(prpartsupp);
	...
	guard<table_scan_iter_impl<partsupp_t> > ps_iter;
	{
	    ...
	    e = _ppartsupp_man->get_iter_for_file_scan(_pssm, tmp_ps_iter);

	    ps_iter = tmp_ps_iter;

	    if(e.is_error()){goto done3;}
	...
	e = ps_iter->next(_pssm, eof, *prpartsupp);
	if(e.is_error()){ goto done3; }
    ...
    _ppartsupp_man->give_tuple(prpartsupp);
    /*
    map<int, pair< decimal, vector<int>* > >::iterator it = minlist.begin();

    while( it != minlist.end()){

	vector<int>::iterator vit = (*(it->second.second)).begin();

	if( it->second.first != -1){
	    cout<<it->first<<" : ";
	    while( vit != (*(it->second.second)).end()){
		cout << (*vit)<<" ";
		vit++;
	    }
	    cout<<endl;
                            ...
                            q3_input_t&  q3in )
{
	...
	guard< table_scan_iter_impl<customer_t> > c_iter;
	{
	    ...
	    e = _pcustomer_man->get_iter_for_file_scan(_pssm, tmp_c_iter);
	...
	e = c_iter->next(_pssm, eof, *prcustomer);
	if( e.is_error() ){ goto done1;}
...
done1:
    ...
    table_row_t* prorder = _porders_man->get_tuple();
    assert( prorder );
	...
	guard< table_scan_iter_impl<orders_t> > o_iter;
	{
	    ...
	    e = _porders_man->get_iter_for_file_scan(_pssm, tmp_o_iter);
	    o_iter = tmp_o_iter;
	    if(e.is_error()){goto done2;}
	}
	bool eof;
	...
	e = o_iter->next(_pssm, eof, *prorder);
	if(e.is_error()){ goto done2; }
 ...
 done2:
    _porders_man->give_tuple(prorder);
	...
	bool eof;
	    ...
	    map<int, q3_order_needed_data>::iterator tmp = ordersdt.find(aline.L_ORDERKEY);
...
done3:
    _plineitem_man->give_tuple(prlineitem);    
    /*
    map<q3_group_by_key_t, double, q3_group_by_comp>::iterator iter = shippingQ.begin();

    while( iter != shippingQ.end() ){

	cout<< iter->first.l_orderkey<<" "<<
	    iter->second<<" "<<
	    iter->first.o_orderdate<<" "<<
	    iter->first.o_shippriority<<endl;
                            ...
                            q4_input_t&  in )
{
    ...
    table_row_t * prorders = _porders_man->get_tuple();
    assert(prorders);
	...
	guard<index_scan_iter_impl<orders_t> > o_iter;
	{
	    ...
	    e = _porders_man->o_get_range_iter_by_index(_pssm,
							tmp_o_iter,
							prorders,
							lowrep,
							highrep,
	...
	e = o_iter->next(_pssm, eof, *prorders);
	if(e.is_error()){ goto done1; }
    ...
    done1:
    _porders_man->give_tuple(prorders);
...
done2:
    ...
    /*
    cout<<"output: "<<endl;
    map<int,int>::iterator it = priority_count.begin();
    
    while(it != priority_count.end()){
                            ...
                            q5_input_t& q5in)
{
	...
	while(!eof){
		...
		customer_nation.insert( pair<int,int>( acust.C_CUSTKEY, acust.C_NATIONKEY));
...
done1:
	...
	while(!eof){
		...
		ordersK_cust.insert(pair<int,int> (anorder.O_ORDERKEY, anorder.O_CUSTKEY));
 ...
 done4:
	...
	bool eof;
		...
		map<int,int>::iterator custiter = customer_nation.find(orderiter->second);
...
done5:
    _plineitem_man->give_tuple(prlineitem);
    /*
    map<int,double>::iterator it = nation_rev.begin();

    char n[55];
    while(it != nation_rev.end()){
	nation_to_str( it->first, n);
                            ...
                            q7_input_t&  q7in )
{
	...
	while(!eof){
	    ...
	    if(acust.C_NATIONKEY == q7in.n_name1 ||
	       acust.C_NATIONKEY == q7in.n_name2)
		cust_nationK.insert(pair<int,int>
				    (acust.C_CUSTKEY, acust.C_NATIONKEY));
...
done1:
	...
	while(!eof){
		...
		orderk_custk.insert(pair<int,int>(anorder.O_ORDERKEY, anorder.O_CUSTKEY));
 ...
 done3:
    ...
    start_shipdate.tm_year = 95; start_shipdate.tm_mon = 0; start_shipdate.tm_mday = 1;
    time_t s_ship = mktime(&start_shipdate);
    
    struct tm end_shipdate;
    end_shipdate.tm_year = 96; end_shipdate.tm_mon = 11; end_shipdate.tm_mday =31;
	...
	while (!eof) {
		...
		if(cust != cust_nationK.end()){
			...
			map<q7_group_by_key_t, double, q7_group_by_comp>::iterator it =
			    vol_shipping.find
			    (q7_group_by_key_t(supp->second, cust->second, date.tm_year));
			...
			if( it != vol_shipping.end()){
			    ...
			    c+=  price;
			    vol_shipping.insert
				(pair<q7_group_by_key_t, double>(q7_group_by_key_t(supp->second, cust->second, date.tm_year ), c ));
			}else{

			    vol_shipping.insert
				(pair<q7_group_by_key_t, double>
				 (q7_group_by_key_t(supp->second, cust->second,
						    date.tm_year ), price ));
			}
		    } 
		}
 ...
 done4:
    ...
    /*
    map<q7_group_by_key_t, double, q7_group_by_comp>::iterator it = vol_shipping.begin();

    while(it != vol_shipping.end()){

	int s_nation = it->first.supp_nation;
	int c_nation = it->first.cust_nation;
	int l_year = it-> first.l_year;

	char sn[50]; nation_to_str(s_nation , sn );
	char cn[50]; nation_to_str(c_nation , cn );
	
	cout<<sn<<" "<<cn<<" "<<l_year<<" "<<it->second<<endl;
	it++;
    }
    */
                            ...
                            q8_input_t&  q8in )
{
    ...
    e = _pnation_man->n_index_probe(_pssm, prnation, q8in.n_name);
...
done3:
	...
	while(!eof){
	    ...
	    supp_nation.insert(pair<int,int>(asupplier.S_SUPPKEY, asupplier.S_NATIONKEY));
 ...
 done6:
...

    {
		...
		all_nation.push_back(q8_inner_table(aline.L_SUPPKEY, oit->second, price));
 ...
 done5:
	...
	mkt_share.insert(pair<int,q8_groupe_by_value_t>(all_nation[i].o_year,
							q8_groupe_by_value_t(a,b)));

	
    }
    /*
    map<int,q8_groupe_by_value_t>::iterator itt = mkt_share.begin();

    while(itt != mkt_share.end()){

	cout<<(*itt).first<<" "<<((*itt).second.a/(*itt).second.b)<<endl;
                            ...
                            q9_input_t& q9in  )
{
 ...
 done1:
    ...
    //cout<<"the size : "<<p_keys.size()<<endl;
    //table scan supplier
 ...
 done3:
	...
	tpch_partsupp_tuple apartsupp;
	    ...
	    e = _ppartsupp_man->ps_index_probe(_pssm, prpartsupp,
 ...
 done4:
    _plineitem_man->give_tuple(prlineitem);
    /*
    map <q9_group_by_key_t, decimal, q9_group_by_comp>::iterator it = profit_m.begin();
    cout<<"the size : "<<profit_m.size()<<endl;
    char n[50];
    while( it != profit_m.end()){
	nation_to_str((*it).first.nation_k, n);
	cout<<n<<" "<<(*it).first.year<<endl;
	it++;
    }
    */
                             ...
                             q10_input_t&  q10in )
{
	...
	while(!eof){
		...
		map<int, vector<int>* >::iterator it = cust_ordersK.find(anorder.O_CUSTKEY);
		...
		if( it != cust_ordersK.end() ){
		    ...
		    cust_ordersK.insert(pair<int, vector<int>*>
					(anorder.O_CUSTKEY, v));
		}
	    }
    ...
    _porders_man->give_tuple(prorder);
    /*
    cout<<"num of order : "<<orders_price.size()<<endl;
    map<int, double>::iterator oit = orders_price.begin();
    while(oit != orders_price.end()){
	cout<<oit->first<<endl;
	oit++;
    }
    */	
 ...
 done2:
	...
	e = _pcustomer_man->c_index_probe(_pssm, prcustomer, cit->first);
	...
	customer_rev[q10_group_by_key_t
		     (cit->first, acust.C_NAME, acust.C_ACCTBAL, acust.C_PHONE, acust.C_NATIONKEY, acust.C_ADDRESS, acust.C_COMMENT)] = rev;
	
	cit++;
    }
    /*
    map<q10_group_by_key_t, double, q10_group_by_key_comp>::iterator it  =
	customer_rev.begin();

    while( it != customer_rev.end()){
	cout<<it->first.c_custkey<<" "<<it->second<<endl;
	it++;
    }
    */
                             ...
                             q11_input_t& q11in )
{
 ...
 done1:
    ...
    //cout<<" num of supp : "<<suppK.size()<<endl;
	...
	while(!eof){
		...
		map<int,decimal>::iterator pit =partK_val.find
		    (apartsupp.PS_PARTKEY);
    ...
    //cout<<"num of part "<<partK_val.size()<<endl;
     done3:
    
    _ppartsupp_man->give_tuple(prpartsupp);
    /*
    map<int, decimal>::iterator it = partK_val.begin();
    cout<<partK_val.size()<<endl;
    while(it != partK_val.end()){

	if( it->second > totalval){
	    cout<<it->first<<endl;
	}
	it++;
    }
                             ...
                             q12_input_t& q12in)
{
   ...
   vector<pair<int, int> > orderK_shipmode;
    map<int, pair<int,int> > shpmd_HLC_LLC; //shipmode -> high_line_count, low_line_count
	...
	{
	    index_scan_iter_impl<lineitem_t>* tmp_l_iter;
	    e = _plineitem_man->l_get_range_iter_by_receiptdate_index(_pssm,
								      tmp_l_iter,
								      prlineitem,
								      lowrep,
								      highrep,
	...
	while(!eof){
		...
		if(commitdate < receiptdate && shipdate < commitdate)
		    orderK_shipmode.push_back(pair<int,int> (aline.L_ORDERKEY, shipmode) );
...

	{
	    ...
	    {
		index_scan_iter_impl<orders_t>* tmp_o_iter;
		e = _porders_man->o_get_iter_by_index(_pssm,
						     tmp_o_iter,
						     prorder,
						     low_rep,
						     high_rep,
	...
	//output the result
	/*	
    cout << q12in.l_shipmode1 << " " <<
	shpmd_HLC_LLC[q12in.l_shipmode1].first<<" "<<
	     shpmd_HLC_LLC[q12in.l_shipmode1].second<<endl;
			     
    cout << q12in.l_shipmode2 << " " <<
	shpmd_HLC_LLC[q12in.l_shipmode2].first<<" "<<
	     shpmd_HLC_LLC[q12in.l_shipmode2].second<<endl;
	*/
 done:
...
done1:
 ...
 done2:
    ...
    for(map<int,int>::iterator iter = c_orders.begin(); iter != c_orders.end(); iter++){
    ...
    /*
    //output some part of the result
    int i=0;
    map<int,int>::iterator iter = o_count_cust.begin();
    while(iter != o_count_cust.end() && i <10 ){
                             ...
                             q14_input_t& q14in)
{
	...
	{
	    index_scan_iter_impl<lineitem_t>* tmp_l_iter;
	    e = _plineitem_man->l_get_range_iter_by_index(_pssm,
						       tmp_l_iter,
						       prlineitem,
						       lowrep,
						       highrep,
	...
	while(!eof){
	    ...
	    map<int, vector<float>*>::iterator pvector = pKey_prices.find( aline.L_PARTKEY );
		...
		pvector->second->push_back( theprice );
	       ...
	       pKey_prices.insert(pair<int, vector<float>* >
				       (aline.L_PARTKEY, v));
	    }
    ...
    //cout<<promorevenue/totalrevenue<<endl;
                             ...
                             q15_input_t&  q15in )
{
    ...
    for(map<int,float>::iterator iter = stream_id.begin(); iter != stream_id.end(); iter++)
	...
	while(!eof){
	   ...
	   if( brand != q16in.p_brand &&  type != q16in.p_type){
		   ...
		   if( q16in.p_size[i] == psize){
		       pKeys_type[apart.P_PARTKEY] =
		          required_type(brand, psize, type);
		       break;
	   	   }
	   }	    		
	   e = p_iter->next(_pssm, eof, *prpart);
                             ...
                             q17_input_t& q17in )
{
	...
	while(!eof){
	    ...
	    int container = str_to_containers1(s1)*10+str_to_containers2(strtok(NULL," "));
...
done2:
    ...
    //    cout<<"rev : "<<revenue<<endl;
    // l_discount is more than 1, that's why revenue is negetive
    return (e);
}// EOF: Q19
                             ...
                             q20_input_t& q20in )
{
 ...
 //phase#3 indexscan lineitem: l_shipdate
...

    {
	   ...
	   if( it != supppart_avquant_sumquant.end()){
	       ...
	       c+= aline.L_QUANTITY;
	       supppart_avquant_sumquant[ pair<int,int>(aline.L_PARTKEY, aline.L_SUPPKEY)]=
 ...
 done2:    
    ...
    map< pair<int,int>, pair<int, int> > :: iterator it = supppart_avquant_sumquant.begin();
    ...
    for(map<int,bool>::iterator iter = suppkey.begin(); iter != suppkey.end(); iter++){
...
done1:
    ...
    //    cout<<"avg : "<<ckey_acbalCcode.size()<<endl;
    
    //phase#2 index scan order

    map<int, pair<int, decimal> > cntrycode_scalars; //scalar: numof customer, total balance
	...
	{
	    index_scan_iter_impl<orders_t>* tmp_o_iter;
	    e = _porders_man->o_get_iter_by_findex(_pssm,
							tmp_o_iter,
							prorders,
							lowrep,
							highrep,
    ...
    _porders_man->give_tuple(prorders);
    /*
    map<int, pair<int,decimal> >::iterator iter = cntrycode_scalars.begin();
    
    while( iter != cntrycode_scalars.end()){
