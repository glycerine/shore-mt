        ...
        latch_mode_t latch_mode)
{
    ...
    if (pin_page) {
        DBGTHRD(<<"pin locating page");
        W_DO_GOTO(rc, fi->locate_page(rid, _hdr_page(), latch_mode, true));
                   ...
                   )
{
    ...
    if (pinned()) {
        ...
        DBGTHRD(<<"repin");
        W_DO_GOTO(rc, fi->locate_page(_rid, _hdr_page(), lock_to_latch(_lmode), true));
        W_DO_GOTO(rc, _hdr_page().get_rec(_rid.slot, _rec) );
        w_assert3(_rec);
        if (_start > 0 && _start >= _rec->body_size()) {
            (void) RC(eBADSTART).delegate();
            goto failure;
        }
        _flags = pin_rec_pinned;
